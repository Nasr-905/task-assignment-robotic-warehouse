...
class MissionType(Enum):
    PICKING = 1     # travel to a location (rack or pickerwall) and pick up the shelf there
    RETURNING = 2   # carry a shelf to an empty rack slot and deposit it
    DELIVERING = 3  # carry a shelf to an empty pickerwall goal slot and deposit it
...
def heuristic_episode(env, render=False, seed=None):
    ...
    while not done:
        ...
        for agv in agvs:
            ...
            # [PICKING done] shelf has been loaded onto the AGV
            if mission.mission_type == MissionType.PICKING and mission.at_location and agv.carrying_shelf:
                ...
                if was_at_goal:
                    # Displacement pick-up: return the shelf to an empty rack slot
                    ...
                    if empty_location_ids:
                        ...
                        dists = [len(env.find_path((agv.y, agv.x), yx, agv, care_for_agents=False))
                                 for yx in empty_yx]
                    ...
                    # Exclude goal slots already targeted by another delivering AGV
                    ...
                    # If no slot is free yet the AGV waits; displacement will clear one

            # [DELIVERING done] shelf was deposited at pickerwall; AGV is free
            ...
            # [RETURNING done] shelf was deposited back in the rack; AGV is free
            elif mission.mission_type == MissionType.RETURNING and mission.at_location and not agv.carrying_shelf:
                assigned_agvs.pop(agv)
                assigned_items.pop(agv, None)

        # -- Assign fetch missions: free AGV -> closest unassigned requested shelf --
        for item in request_queue:
            ...
            agv_paths = [env.find_path((a.y, a.x), (item.y, item.x), a, care_for_agents=False)
        ...
        # -- Displacement: send free AGVs to clear non-requested pickerwall slots --
