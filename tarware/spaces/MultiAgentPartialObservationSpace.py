...
class MultiAgentPartialObservationSpace(MultiAgentBaseObservationSpace):
    """With pickers removed, partial and global observations share the same structure.
    Each AGV observes its own state first, then all other AGVs, then shelf/pickerwall state."""

    def __init__(self, num_agvs, grid_size, shelf_locations, num_goals=0, normalised_coordinates=False):
        super(MultiAgentPartialObservationSpace, self).__init__(num_agvs, 0, grid_size, shelf_locations, normalised_coordinates)
